...
public class WorldZipper {
    ...
    public void zip(World world, File worldDirectory, String destZipFile) {
        if(plugin.pluginStatus.contains(world.getName())){
            plugin.getServer().getConsoleSender().sendMessage(plugin.colorize(plugin.tag + plugin.config.getString("alreadyBackup") + ChatColor.WHITE + " " + worldDirectory.getName()));
            return;
        }
            ...
            public void run() {
                ...
                new Thread(() -> {
                    try {
                        ...
                        plugin.getServer().getConsoleSender().sendMessage(plugin.colorize(plugin.tag + plugin.config.getString("finishedBackup") + ChatColor.WHITE + " " + worldDirectory.getName()));
                        ...
                        e.printStackTrace();
                        plugin.getServer().getConsoleSender().sendMessage(plugin.colorize(plugin.tag + plugin.config.getString("failedBackup") + ChatColor.WHITE + " " + worldDirectory.getName()));
                    }
